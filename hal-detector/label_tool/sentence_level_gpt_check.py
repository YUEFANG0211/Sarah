...
def main():
    ...
    with open(args.input_file, 'r', encoding='utf-8') as infile, \
         open(args.groundtruth_file, 'r', encoding='utf-8') as gtfile, \
         open(args.output_file, 'w', encoding='utf-8') as outfile:
        
        for line_infile, line_gtfile in tqdm(zip(infile, gtfile), desc="Processing", unit="lines"):
            lvlm_output = json.loads(line_infile.strip())  # 解析每一行来自 infile 的 JSON
            dataset_information = json.loads(line_gtfile.strip())
            
            id = lvlm_output.get("id", "")
            instruction = lvlm_output.get("instruction", "")
            text = lvlm_output.get("most_likely_generation","")
            gt = dataset_information.get("ground truth","")
