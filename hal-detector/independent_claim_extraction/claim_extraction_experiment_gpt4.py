...
class ClaimsExtractor:
    ...
    def claims_from_text(self, text: str, tokens: List[int]) -> List[Claim]:
        sentences = []
        ...
        for s in sentences:
            ...
            sent_start_token_idx += sent_start_idx
            sent_end_token_idx += sent_end_idx
             
            for c in self._claims_from_sentence(
                s, tokens[sent_start_token_idx:sent_end_token_idx]
            ):
    ...
    def _claims_from_sentence(
        ...
        for claim_text in extracted_claims.split("\n"):
            ...
            if match_string is None:
                continue
            aligned_token_ids = self._align(sent, match_string, sent_tokens)
    ...
    def _align(
        ...
        while sent_pos < len(sent) and cur_token_i < len(sent_tokens):
            cur_token_text =sent_tokens[cur_token_i]
...
def load_dependencies_from_jsonl(line: str) -> Dict[str, object]:
    ...
    greedy_tokens = [item["token"] for item in data.get("prob", {}).get("content", [])]
...
def main():
    ...
    with open(input_file, 'r', encoding='utf-8') as file:
        ...
        for line in tqdm(lines, desc="Processing claims", unit="line"):
            ...
            result = claims_extractor(dependencies)
