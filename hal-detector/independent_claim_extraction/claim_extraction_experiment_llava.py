    ...
    def claims_from_text(self, text: str, tokens: List[int], tokenizer) -> List[Claim]:
        ...
        for s in sentences:
            ...
            for c in self._claims_from_sentence(
                ...
                # for i in range(len(c.aligned_token_ids)):    
                    # text = tokenizer.decode(tokens[c.aligned_token_ids[i]])
                    # print(text)
                    # print(c.aligned_token_ids[i])
...
def load_input_file(line: str) -> Dict[str, List]:
    greedy_texts = []
    greedy_tokens = []
    
    data = json.loads(line.strip())  # 解析每一行 JSON 数据
    
    # 确保 'text' 字段是字符串类型
    greedy_texts.append(str(data["text"]))  # Add "text" field to greedy_texts
    
    # 确保 'token_id' 是整数列表（如果有必要的话进行转换）
    tokens = data["token_id"]
    if isinstance(tokens, list):
        # 确保 token_id 列表中的所有项都是整数
        tokens = [int(token) for token in tokens if isinstance(token, int)]
    else:
        # 如果不是列表，假设它是一个单一的 token，将其转换为列表
        tokens = [int(tokens)]  # Convert token_id to a list with a single integer
    
    greedy_tokens.append(tokens)  # Add the list of token ids to greedy_tokens
...
def load_dependencies_from_jsonl(line: str) -> Dict[str, object]:
    ...
    greedy_tokens = [item.get("token_id") for item in token_data if "token_id" in item]  # 提取每个字典中的 "token_id"
...
def main():
    args = parse_args()
    # Initialize the ClaimsExtractor
    ...
    with open(input_file, 'r', encoding='utf-8') as file:
        ...
        for line in tqdm(lines, desc="Processing claims", unit="line"):
            # 读取当前行的数据
